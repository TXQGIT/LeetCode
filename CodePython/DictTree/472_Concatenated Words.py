...
class Solution:
    def findAllConcatenatedWordsInADict(self, words):
        # 当有匹配的单词时，将余下单词从头进行匹配
        # def search(root, word, splited):
            ...
            #         if i==len(word)-1:
            #             return splited>=1
            #         if search(tire.root, word[i+1:], splited+1):
            #             return True
            # return False

        def search(root, word, splited, path):
            if len(word) == 0:
                return splited > 1
            flag = False
            cur = word[0]
            if cur in root.children:
                root = root.children[cur]
                path += cur
                if root.is_leaf and search(tire.root, word[1:], splited + 1, ''):
                    print(path)
                    return True
                flag |= search(root, word[1:], splited, path)
            return flag
...
words = ["rfkqyuqfjkx", "", "vnrtysfrzrmzl", "gfve", "qfpd", "lqdqrrcrwdnxeuo", "q", "klaitgdphcspij", "hbsfyfv",
         ...
         "nuqhmfj", "qtg", "cwkuzya mnerp", "fmwevhwlezo", "ye", "hbrcewjxvcezi", "tiq", "tfsrptug", "iznorvonzjfea",
...
solotion = Solution()
print(solotion.findAllConcatenatedWordsInADict(words))