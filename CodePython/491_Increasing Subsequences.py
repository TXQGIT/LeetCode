class Solution:
    def findSubsequences(self, nums):
        ...
        # 加入重复数字后，比如序列 [4, 6, 7, 7][4,6,7,7] 遍历完前三个数字后可得到 [[4], [4, 6], [6], [4, 7], [4, 6, 7], [6, 7], [7]][[4],[4,6],[6],[4,7],[4,6,7],[6,7],[7]]，再加入 7 的话与前三个的组合就会和后面发生重复，所以要避免与前三个组合。利用字典保存该数字上一次出现时未组合之前结果的长度，当再次出现时只和该索引以及之后的序列进行组合，这样就避免了重复。最后去掉长度不足 2 的序列
        if len(nums) == 0:
            return []
        res = [[nums[0]]]
        idx_map = {nums[0]: -1}
